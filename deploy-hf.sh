...
prepare_backend_for_hf() {
  ...
  cp apps/backend/requirements-prod.txt "$HF_DIR/requirements.txt" || { echo "❌ requirements-prod.txt not found"; exit 1; }
...
display_hf_deployment_instructions() {
  ...
  echo "      - requirements.txt (from apps/backend/requirements-prod.txt)"
